...
export function updateState(delta) {
  ...
  if (getGameState() === GameState.PLAYING && !blackoutTriggered && camera.position.z < -24) {
  ...
  if (getGameState() === GameState.PLAYING) {
    ...
    if (isPlayerHidden) {
      ...
      if (keys.has("Space")) {
        ...
        window.p1BreathStamina = Math.max(0, p1BreathStamina - delta * 24);
        window.fear = Math.max(0, fear - delta * 4);
      } else {
        window.isHoldingBreath = false;
        window.p1BreathStamina = Math.min(100, p1BreathStamina + delta * 18);
        if (scene.userData.meeraCharacter && !godModeActive) {
          ...
          if (distToMeera < 8.0) {
            window.fear = Math.min(100, fear + delta * 8.5);
          }
        }
      }
    ...
    if (coopMode && camera2) {
      if (isPlayer2Hidden) {
        ...
        if (player2Keys.has("Period")) {
          ...
          window.p2BreathStamina = Math.max(0, p2BreathStamina - delta * 24);
          window.fear2 = Math.max(0, fear2 - delta * 4);
        } else {
          window.isHoldingBreath2 = false;
          window.p2BreathStamina = Math.min(100, p2BreathStamina + delta * 18);
          if (scene.userData.meeraCharacter && !godModeActive) {
            const distToMeera2 = camera2.position.distanceTo(scene.userData.meeraCharacter.position);
            if (distToMeera2 < 8.0) {
              window.fear2 = Math.min(100, fear2 + delta * 8.5);
            }
          }
        }
  ...
  if (flashlightOn && !infiniteBatteryActive) window.battery = Math.max(0, battery - delta * 0.85 * batteryMultiplier);
  if (battery <= 0 && flashlightOn) setFlashlight(false);

  if (window.emfActive && !infiniteBatteryActive) {
    window.battery = Math.max(0, battery - delta * 0.46 * batteryMultiplier);
  ...
  if (window.emfActive) {
    ...
    if (emfTickTimer <= 0) {
      ...
      window.emfTickTimer = intervals[Math.min(emfLevel - 1, 4)];
      if (audioManager) {
  ...
  if (coopMode && window.emfActive2) {
    ...
    if (emfTickTimer2 <= 0) {
      ...
      window.emfTickTimer2 = intervals2[Math.min(emfLevel2 - 1, 4)];
      if (audioManager) {
  ...
  if (godModeActive) {
    ...
    window.fear = THREE.MathUtils.lerp(fear, depthFear + darknessFear + inspected * 5, delta * 0.9);
    if (fear >= 100 && getGameState() === GameState.PLAYING) {
    ...
    if (sanityDrain > 0) {
      window.p1Sanity = Math.max(0, p1Sanity - delta * sanityDrain);
    } else {
      if (currentLevel === 1 && camera.position.z > -12) {
        window.p1Sanity = Math.min(100, p1Sanity + delta * 2.0);
      }
    }
  }
  ...
  window.p1HeartRate = THREE.MathUtils.lerp(p1HeartRate, targetHR1 + Math.sin(clock.getElapsedTime() * 3) * 1.5, delta * 2.0);
  ...
  if (p1LockerMinigameActive) {
    ...
    window.p1LockerMinigameProgress = Math.max(-50, p1LockerMinigameProgress - delta * 4);
  ...
  if (coopMode && camera2 && player2Flashlight) {
    if (flashlightOn2 && !infiniteBatteryActive) window.battery2 = Math.max(0, battery2 - delta * 0.85 * batteryMultiplier);
    if (battery2 <= 0 && flashlightOn2) setFlashlight2(false);

    if (emfActive2 && !infiniteBatteryActive) {
      window.battery2 = Math.max(0, battery2 - delta * 0.46 * batteryMultiplier);
    ...
    if (godModeActive) {
      ...
      window.fear2 = THREE.MathUtils.lerp(fear2, depthFear2 + darknessFear2 + inspected * 5, delta * 0.9);
      if (fear2 >= 100 && getGameState() === GameState.PLAYING) {
      ...
      if (sanityDrain2 > 0) {
        window.p2Sanity = Math.max(0, p2Sanity - delta * sanityDrain2);
      } else {
        if (currentLevel === 1 && camera2.position.z > -12) {
          window.p2Sanity = Math.min(100, p2Sanity + delta * 2.0);
        }
      }
    }
    ...
    window.p2HeartRate = THREE.MathUtils.lerp(p2HeartRate, targetHR2 + Math.sin(clock.getElapsedTime() * 3) * 1.5, delta * 2.0);
    ...
    if (p2LockerMinigameActive) {
      ...
      window.p2LockerMinigameProgress = Math.max(-50, p2LockerMinigameProgress - delta * 4);
    ...
    const batteryText2 = document.getElementById("battery-p2-val");
    const batteryMeter2 = document.getElementById("battery-p2-meter");
    const batteryPanelP2 = document.getElementById("battery-p2-panel");
    if (batteryText2) {
    ...
    const fearText2 = document.getElementById("fear-p2-val");
    const fearMeter2 = document.getElementById("fear-p2-meter");
  ...
  const batteryText = document.getElementById("battery-p1-val");
  const batteryMeter = document.getElementById("battery-p1-meter");
  const batteryPanelP1 = document.getElementById("battery-p1-panel");
  if (batteryText) {
  ...
  const fearText = document.getElementById("fear-p1-val");
  const fearMeter = document.getElementById("fear-p1-meter");
  ...
  if (activeFear > statFearPeak) window.statFearPeak = activeFear;
  if (vignette) {
  ...
  if (scene.userData.meeraCharacter) {
    ...
    if (meeraState !== AiState.INACTIVE) {
      if (lockerAlertState && lockerTargetToInspect) {
        let meeraSpeed = 1.65 * meeraSpeedMultiplier;
        ...
        const targetSprintExhausted = targetCamera ? (targetIsP2 ? sprintExhausted2 : window.sprintExhausted) : false;
        ...
        if (meeraState === AiState.PATROL) {
          let meeraSpeed = 1.2 * meeraSpeedMultiplier;
          ...
          let meeraSpeed = ((currentLevel === 2 ? 1.48 : 1.6) + (targetFear / 160)) * meeraSpeedMultiplier;
          ...
          if (targetIsP2) {
            if (!godModeActive) window.fear2 = Math.min(100, fear2 + delta * 3.6);
          } else {
            if (!godModeActive) window.fear = Math.min(100, fear + delta * 3.6);
          }
        ...
        if (targetCamera && targetDist < 4.5 && meeraState === AiState.CHASE) {
          if (targetIsP2) {
            if (!godModeActive) window.fear2 = Math.min(100, fear2 + delta * 24);
          } else {
            if (!godModeActive) window.fear = Math.min(100, fear + delta * 24);
          }
  ...
  if (!meeraFirstWhisperPlayed && inspected === 0 && camera.position.z < -24.5 && getGameState() === GameState.PLAYING) {
  ...
  if (!meeraSecondEventPlayed && inspected >= 1 && camera.position.z < -16 && getGameState() === GameState.PLAYING) {
  ...
  if (!meeraFinalEventPlayed && inspected === 3 && camera.position.z < -24.0 && getGameState() === GameState.PLAYING) {
  ...
  if (activeApparitionWalk && apparitionGhost) {
    ...
    apparitionGhost.position.y = Math.sin(clock.getElapsedTime() * 3) * 0.04;
